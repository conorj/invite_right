class EventRange
  class << self
    # following method gets array of days meetings are on
    # and creates new events for these
    def add_events(params)
      ...
      if date_start == date_end
        ...
        if params[:repeats] == 'fortnightly'
          dates = dates.select.with_index { |_, i| i.odd? }
        end
      end
      events = []
      dates.each do |date|
        dtime = date.to_datetime + event_time.seconds_since_midnight.seconds
        events << Event.add_invite(
          date: dtime,
          place: params[:place],
    ...
    def get_dates(date_range, day_to_use)
      filter_day = date_start.send(day_to_use.to_sym)
